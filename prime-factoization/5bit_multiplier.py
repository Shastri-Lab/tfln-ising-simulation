...
from AND_FA_Hamiltonians import AND, FA, generate_Nbit_multiplier_Hamiltonian
import random
...
def simulated_annealing(objective,x,temperature = 25, N_iters = 1000000, inputs = None,
    ...
    for n in range(N_iters):
        ...
        new_candidate[Nbits_in*4 + Nbits_in ** 2] = -1
        new_candidate[Nbits_in*4 + Nbits_in ** 2 + Nbits_in * 2] = -1
        new_candidate[Nbits_in*4 + Nbits_in ** 2 + Nbits_in * 4] = -1
        new_candidate[Nbits_in*4 + Nbits_in ** 2 + Nbits_in * 6] = -1
        # new_candidate[Nbits_in*4 + Nbits_in ** 2 + Nbits_in * 8] = -1
...
for n in range(101):
    ...
    x, f, l = simulated_annealing(Hamiltonian,spin_vec_0,outputs=outputs,n_flips=1, N_iters=int(5e6), N_updates_btw_anneal = 50000)
    # x, f, l = simulated_annealing(Hamiltonian,spin_vec_0,inputs=A+B)
    # print(f, Hamiltonian(x))
    A_ = (x[:Nbits_in] + 1) // 2
