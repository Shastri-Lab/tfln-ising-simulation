...
* Description: Vitis code to give input of Model with decesion
* 
* Dependencies: 
* 
* Revision:
...
int main() {
    ...
    while (1) {
    	...
    	if(data == WRITE){
    		...
    		scanf("%d",&data);
    		Xil_Out32((XPAR_AXI_DECISION_0_BASEADDR) + (reg_num*AXI_DATA_BYTE), (u32)(data));
    		printf("register write done register_number (%d), value : %d\r\n", reg_num, data);

    	} else if (data == READ){
    		printf("please input READ register number (0~3)\n");
    		scanf("%d",&reg_num);
    		data = Xil_In32((XPAR_AXI_DECISION_0_BASEADDR) + (reg_num*AXI_DATA_BYTE));
