...
METHODS = {
    'numpy': numpy_dgemv,
    'mps': load('./libgpu_mps_float.dylib', 'sgemv_mps',
                    None, [pF, pF, pF, ctypes.c_size_t, ctypes.c_size_t]),
    'blas': load('./libcpu_blas_float.so', 'sgemv_blas',
                 None, [pF, pF, pF, ctypes.c_size_t, ctypes.c_size_t]),
    'ref':  load('./libcpu_ref_float.so',  'sgemv_ref',
                 None, [pF, pF, pF, ctypes.c_size_t, ctypes.c_size_t]),
}
...
def run_benchmarking_test(m, n, repeats=10_000, skip=False):
    ...
    for method, fn in METHODS.items():
        ...
        if method == 'numpy':
            ...
            warm_args = (A.ctypes.data_as(pF), x.ctypes.data_as(pF),
                         y.ctypes.data_as(pF), m, n)

        # warm‑up
        bench(method, fn, *warm_args)

        deltas = np.zeros(repeats, dtype=np.float32)
