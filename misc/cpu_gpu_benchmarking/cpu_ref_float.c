...
// Basic Linear Algebra Subprograms (BLAS) function sgemv (single precision
// general matrix-vector multiplication) using C. It provides a function
// to perform matrix-vector multiplication (sgemv) using double precision
...
clang -O0 \                         # Optimization level 0 because I want it to go as slow as possible
      ...
      -o libcpu_ref_float.so cpu_ref_float.c    # Output shared library name

*/

#include <stddef.h>

void sgemv_ref(const float *A, const float *x,
